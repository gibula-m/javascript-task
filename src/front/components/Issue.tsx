...
export function Issue(props : any) {
  let stateDetails: { text: string, color: string } = { text : "Unknown", color: "secondary" };
  if(props.item.state != null){
    stateDetails = issueState.get(props.item.state);
  }
  return <div className="col-12"><div className="card" style={{marginBottom:"10px"}}>
            <div className="card-body">
            <div className="">
              <h5 className="card-title d-inline mr-3">{props.item.title}</h5>
              <span className={"d-inline badge badge-" + stateDetails.color}>{stateDetails.text}</span>
            </div>
